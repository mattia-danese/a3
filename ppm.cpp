  ...
  if (ppmFile.is_open()) {
	  ...
	  while (getline(ppmFile, line)) {
		  ...
		  // Read in dimensions
			  ...
			  if (width > 0 && height > 0) {
				  color = new unsigned char[width*height * 3];
				  if (color == NULL) {
...
SceneColor ppm::getPixel(int s, int t){
	SceneColor ret;
	if(s > width || t > height){
		ret.r = 0;
		ret.g = 0;
    ...
    ret.r = color[(s*3)+height*(t*3)];
    ret.g = color[(s*3)+height*(t*3)+1];
    ret.b = color[(s*3)+height*(t*3)+2];
	return ret;
  }
}
